...
async function printReceipt(appt: AppointmentExtended) {
  ...
  const logoHtml = logoUrl
    ? `<img src="${logoUrl}" alt="${siteTitle}" style="max-height:80px;max-width:220px;object-fit:contain;display:block;margin:0 0 4px" />`
    : `<div style="font-size:32px;font-weight:900;color:#fff;letter-spacing:-0.5px">❤️ ${siteTitle}</div><div style="font-size:12px;font-weight:500;color:rgba(255,255,255,0.75);margin-top:2px">${doctorTitle || "Cardiac Care"}</div>`
  ...
  <div class="body">
    ...
    <div class="section">
      ...
      <div class="grid2">
        ...
        ${(appt.appointmentFee && Number(appt.appointmentFee) > 0) ? `
        <div class="field">
          <div class="fl">Consultation Fee</div>
          <div class="fv">₹${appt.appointmentFee}</div>
        </div>
        <div class="field">
          <div class="fl">Payment Status</div>
          <div class="fv">
...
async function sendAppointmentNotification(
  apptId: string, doctorId: string, patientName: string,
  date: string, time: string, reason: string,
) {
  // Fetch doctor's FCM token from their user document
  const { doc: fsDoc, getDoc: fsGetDoc } = await import('firebase/firestore')
  const { db: fsDb } = await import('@/lib/firebase')
  const userSnap = await fsGetDoc(fsDoc(fsDb, 'users', doctorId))
  const fcmToken = userSnap.exists() ? userSnap.data()?.fcmToken : null
  if (!fcmToken) {
    console.warn('Doctor has no FCM token stored — skipping push notification')
    return
  }
  // Call the backend notification server
  await fetch('/api/send-notification', {
    ...
    body: JSON.stringify({
      ...
      body: `${patientName} — ${date} at ${time}`,
      data: { screen: 'appointments', apptId, reason },
    }),
  })
}
...
function AddAppointmentModal({ onClose, onAdded, existingAppointments }: AddModalProps) {
  ...
  // Load doctors from users collection (role === 'doctor')
  useEffect(() => {
    getDocs(query(collection(db, 'users'), where('role', '==', 'doctor'))).then(snap => {
  ...
  async function searchPatients(term: string) {
    ...
    setLookingUp(true)
    try {
      // Fetch matches (no compound orderBy — avoids composite index requirement).
      // We sort client-side by createdAt desc so the most recent appointment
      // (most complete patient data) wins when deduplicating by phone.
      const snap = await getDocs(query(collection(db, 'appointments'),
        where('patientPhone', '>=', term), where('patientPhone', '<=', term + '\uf8ff'), limit(30)))
      ...
      sorted.forEach(d => {
        ...
        if (!seen.has(data.patientPhone)) {
          ...
          results.push({
            name: data.patientName, phone: data.patientPhone, email: data.patientEmail,
            age: data.age, gender: data.gender, bloodGroup: data.bloodGroup,
            address: data.address, city: data.city, pincode: data.pincode,
            emergencyContact: data.emergencyContact, knownConditions: data.knownConditions,
            currentMedications: data.currentMedications, allergies: data.allergies,
            referredBy: data.referredBy,
          })
        }
      })
      setHints(results.slice(0, 5)); setShowHints(results.length > 0)
  ...
  function pickHint(h: PatientHint) {
    setPatientName(h.name)
    setPatientPhone(h.phone)
    setPatientEmail(h.email ?? '')
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Auto-generate patient reference ID: UBH-XXXXXX (6 alphanumeric chars)
      const generatedPatientId = `UBH-${Math.random().toString(36).slice(2,8).toUpperCase()}`
      payload.patientId = generatedPatientId
      ...
      if (knownConditions.trim()) payload.knownConditions = knownConditions.trim()
      if (currentMedications.trim()) payload.currentMedications = currentMedications.trim()
      if (allergies.trim()) payload.allergies = allergies.trim()
      if (referredBy.trim()) payload.referredBy = referredBy.trim()
      ...
      const added: AppointmentExtended = {
        ...
        patientId: generatedPatientId, doctorId: doctorId || undefined,
      ...
      if (doctorId) {
        sendAppointmentNotification(apptId, doctorId, patientName.trim(), date, time, finalReason)
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-[640px] max-w-[95vw] max-h-[92vh] overflow-auto">
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="relative">
                <label className="block text-xs font-medium text-gray-500 mb-1.5">Phone * <span className="text-gray-300 font-normal">(search existing patient)</span></label>
                ...
                {lookingUp && <span className="absolute right-3 top-9 text-xs text-gray-400">Searching…</span>}
                {showHints && hints.length > 0 && (
                  <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-xl shadow-lg z-20 mt-1 divide-y divide-gray-50">
                    {hints.map(h => (
                      <button key={h.phone} type="button" onClick={() => pickHint(h)}
                        ...
                        <p className="text-xs text-gray-400">{h.phone}{h.age ? ` · ${h.age}y` : ''}{h.bloodGroup ? ` · ${h.bloodGroup}` : ''} · Returning patient</p>
                      </button>
                    ))}
                    <button type="button" onClick={() => setShowHints(false)}
                      className="w-full text-left px-3 py-2 text-xs text-gray-400 hover:bg-gray-50 transition">
                      + New patient — enter details below
                    </button>
                  </div>
                )}
                {existingMatch && <p className="text-xs text-green-600 mt-1">✓ Returning patient — details auto-filled</p>}
              </div>
              <div>
          ...
          {/* Address & Location */}
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Address & Location</p>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input value={pincode} onChange={e => setPincode(e.target.value)} placeholder="e.g. 500001"
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1.5">Emergency Contact <span className="text-gray-300">(Name &amp; Phone)</span></label>
              ...
              <div>
                ...
                <input value={knownConditions} onChange={e => setKnownConditions(e.target.value)}
                  placeholder="e.g. Hypertension, Diabetes Type 2, CAD"
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1.5">Current Medications</label>
                  <input value={currentMedications} onChange={e => setCurrentMedications(e.target.value)}
                    placeholder="e.g. Metoprolol, Aspirin"
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
                </div>
                <div>
              ...
              <div>
                ...
                <input value={referredBy} onChange={e => setReferredBy(e.target.value)}
                  placeholder="e.g. Dr. Arjun Mehta / Self / Online"
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
            </div>
          </div>
          ...
          {/* Assign Doctor */}
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Assign Doctor</p>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">Select Doctor *</label>
              <select value={doctorId} onChange={e => {
                const sel = doctors.find(d => d.uid === e.target.value)
                setDoctorId(e.target.value)
                setDoctorName(sel?.name ?? '')
              }}
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
              ...
              {doctors.length === 0 && (
                <p className="text-xs text-amber-600 mt-1">⚠ No doctors found. Add doctors first via Team page.</p>
              )}
            </div>
          </div>
...
interface DrawerProps {
  ...
  onEditRequest: (appt: AppointmentExtended) => void
}

function AppointmentDrawer({ appointment: appt, allAppointments, onClose, onUpdate, onEditRequest }: DrawerProps) {
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-[500px] bg-white h-full shadow-2xl flex flex-col">
        ...
        <div className="px-6 py-3 bg-gray-50 border-b border-gray-100 flex gap-2 flex-wrap">
          ...
          <button onClick={() => onEditRequest(appt)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border bg-white text-gray-600 border-gray-200 hover:border-blue-300 transition">
            ✏️ Edit
          </button>
        </div>
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 rounded-xl p-4 space-y-2.5">
                <div className="flex items-center justify-between">
                  ...
                  {appt.patientId && (
                    <span className="text-xs font-mono bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full border border-blue-100">
                      {appt.patientId}
                    </span>
                  )}
                </div>
                ...
                {appt.age && <div className="flex items-center gap-2 text-sm text-gray-700"><Icon.User /><span>{appt.age} years · {appt.gender ?? 'gender not specified'}{appt.bloodGroup ? ` · ${appt.bloodGroup}` : ''}</span></div>}
                {(appt.address || appt.city) && (
                  <div className="flex items-start gap-2 text-sm text-gray-700">
                    ...
                    <span>{[appt.address, appt.city, appt.pincode].filter(Boolean).join(', ')}</span>
                  </div>
                )}
                {appt.emergencyContact && (
                  <div className="flex items-center gap-2 text-sm text-gray-700">
                    <svg className="w-3.5 h-3.5 flex-shrink-0 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                    <span className="text-xs text-red-600 font-medium">Emergency:</span>
                    <span>{appt.emergencyContact}</span>
                  </div>
                )}
              </div>
              {(appt.knownConditions || appt.allergies || appt.currentMedications || appt.referredBy) && (
                <div className="bg-amber-50 rounded-xl p-4 space-y-2 border border-amber-100">
                  <p className="text-xs font-semibold text-amber-700 uppercase tracking-wide mb-2">Medical Profile</p>
                  {appt.knownConditions && (
                    <div>
                      <p className="text-xs text-gray-400 mb-0.5">Known Conditions</p>
                      <p className="text-sm text-gray-700">{appt.knownConditions}</p>
                    </div>
                  )}
                  {appt.allergies && (
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-red-600 font-medium">{appt.allergies}</p>
                      </div>
                    </div>
                  )}
                  {appt.currentMedications && (
                    <div>
                      <p className="text-xs text-gray-400 mb-0.5">Current Medications</p>
                      <p className="text-sm text-gray-700">{appt.currentMedications}</p>
                    </div>
                  )}
                  {appt.referredBy && (
                    <div>
                      <p className="text-xs text-gray-400 mb-0.5">Referred By</p>
                      <p className="text-sm text-gray-700">{appt.referredBy}</p>
                    </div>
                  )}
                </div>
              )}
...
export default function AppointmentsPage() {
  ...
  const [editAppt, setEditAppt]         = useState<AppointmentExtended | null>(null)
  ...
  const filtered = useMemo(() => {
    ...
    result = [...result].sort((a, b) => {
      // Safely coerce any value (string, Timestamp, undefined) to a comparable string
      ...
      if (sortKey === 'patientName')        { va = toStr(a.patientName);    vb = toStr(b.patientName) }
      else if (sortKey === 'preferredDate') { va = toStr(a.preferredDate);  vb = toStr(b.preferredDate) }
      else if (sortKey === 'status')        { va = toStr(a.status);         vb = toStr(b.status) }
      else /* createdAt */                  { va = toStr(a.createdAt);      vb = toStr(b.createdAt) }
    ...
    <div className="space-y-6">
      ...
      {selectedAppt && !editAppt && (
        <AppointmentDrawer
          ...
          onEditRequest={appt => setEditAppt(appt)}
        />
      )}
      {editAppt && (
        <EditAppointmentModal
          ...
          onUpdated={(id, data) => {
            handleUpdate(id, data as Partial<AppointmentExtended>)
            setEditAppt(null)
          }}
        />
      )}
