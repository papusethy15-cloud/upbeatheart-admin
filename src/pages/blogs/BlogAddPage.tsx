...
import {
  ...
  BookOpen, Tag, FileText, Search, Globe,
} from 'lucide-react'
...
function BlogContentPreview({ html }: { html: string }) {
  ...
  useEffect(() => {
    ...
    // Activate all sliders rendered in the preview
...
export default function BlogAddPage() {
  ...
  const handleSave = async (status: ContentStatus) => {
    ...
    try {
      ...
      toast.success(status === 'draft' ? 'Saved as draft!' : 'Sent for doctor approval!')
      navigate('/blogs')
    } catch (e) {
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
              <BookOpen className="w-5 h-5 text-primary" /> New Blog Post
            </h1>
            <p className="text-xs text-gray-400 mt-0.5">Fill all sections then send for doctor approval.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            <Send className="w-4 h-4" /> {saving ? 'Saving…' : 'Send for Approval'}
          </button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
              <p className="text-xs text-amber-700 font-medium mb-1">📋 Content Approval Workflow</p>
              <p className="text-xs text-amber-600 leading-relaxed">
                After saving, click <strong>Send for Approval</strong>. The doctor will review it in the mobile app and approve before it goes live on the website.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-3">
              ...
              {[
                ...
                { label: 'Content written',        pass: form.content.length > 200 },
              ].map(({ label, pass }) => (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-1">
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  {form.seoDesc || form.excerpt || 'Meta description will appear here. Keep it between 120–160 characters for best results in Google search.'}
                </p>
              </div>
              <p className="text-xs text-gray-400 mt-3">This is an approximate preview. Actual appearance may vary in Google.</p>
            </div>
      ...
      {tab === 'preview' && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="max-w-2xl mx-auto px-8 py-10">
            ...
            {/* Rich content — same CSS as the editor canvas */}
            {form.content ? (
